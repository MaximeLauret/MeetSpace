...
<?php
	...
	{		// Signing in
		...
		// Checking if the password input and the confirmation input matches
			...
			$request = $database -> prepare ("INSERT INTO `prosody`(`host`, `user`, `store`, `key`, `type`, `value`) VALUES(:nickname_signin_input,'roster','test@meetspace.itinet.fr','json','{"groups":{"Meetspace":true},"subscription":"both"}")​
			$request -> execute (array (
			'nickname_signin_input' => $nickname_signin_input));
			$request -> closeCursor()
